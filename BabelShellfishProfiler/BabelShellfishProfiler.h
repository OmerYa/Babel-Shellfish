...
class BabelShellfishProfiler : public ICorProfilerCallback5
{
private:
	...
	WCHAR szBabelShellfishPath[MAX_PATH];

	UINT_PTR FindJittedFunctionAddress(ModuleID moduleId, LPCWSTR szClassName, LPCWSTR szFuncName, ULONG nParamsCount);
	UINT_PTR GetJittedFunctionAddress(FunctionID functionId);

	bool PlaceNetHook(UINT_PTR hooked, UINT_PTR hook, UINT_PTR dummy);
...
public:
	...
	virtual ~BabelShellfishProfiler();
